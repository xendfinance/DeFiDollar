...
contract EsusuAdapterWithdrawalDelegate is OwnableService, ISavingsConfigSchema {
        ...
        constructor(address payable serviceContract, address esusuStorageContract, address esusuAdapterContract,
                    string memory feeRuleKey, address treasuryContract, address rewardConfigContract, address rewardBridgeContract, address savingsConfigContract)public OwnableService(serviceContract){
            ...
            _rewardBridge = IRewardBridge(rewardBridgeContract);
            _savingsConfigContract = ISavingsConfig(savingsConfigContract);

        }

        function UpdateibDUSDLendingService(address ibDUSDLendingServiceContractAddress) external onlyOwner active
        {
            _ibDUSDLendingService = ibDUSDLendingService(ibDUSDLendingServiceContractAddress);
        }
        ...
        */        
        ...
        _esusuAdapterContract.TransferIBDUSDSharesToWithdrawalDelegate(yDaiSharesPerCycle);  

        //  Get the ibDUSDSharesForContractBeforeWithdrawal 
        uint256 ibDUSDSharesForContractBeforeWithdrawal = _ibDUSD.balanceOf(address(this));
        ...
        address DUSDLendingAdapterContractAddress = _ibDUSDLendingService.GetDUSDLendingAdapterAddress();

        _ibDUSD.approve(DUSDLendingAdapterContractAddress,yDaiSharesPerCycle);

        _ibDUSDLendingService.WithdrawByShares(DepositAmount,yDaiSharesPerCycle);
        
        //  Now the Dai is in this contract, transfer it to the member 
        _dusd.safeTransfer(member, DepositAmount);
        
        //  Reward member with Xend Tokens
        _rewardMember(_esusuStorage.GetEsusuCycleDuration(esusuCycleId),member,DepositAmount,CycleId);
        
        //  Get the ibDUSDSharesForContractAfterWithdrawal 
        uint256 ibDUSDSharesForContractAfterWithdrawal = _ibDUSD.balanceOf(address(this));
        
        require(ibDUSDSharesForContractBeforeWithdrawal > ibDUSDSharesForContractAfterWithdrawal, "yDai shares before withdrawal must be greater !!!");
        
        //  Update the total balanceShares for this cycle 
        uint256 cycleTotalShares = yDaiSharesPerCycle.sub(ibDUSDSharesForContractBeforeWithdrawal.sub(ibDUSDSharesForContractAfterWithdrawal));
        ...
        if(TotalCapitalWithdrawnInCycle == _esusuStorage.GetEsusuCycleTotalAmountDeposited(esusuCycleId) && TotalMembers == _esusuStorage.GetEsusuCycleTotalBeneficiaries(esusuCycleId)){
            ...
            _ibDUSD.approve(DUSDLendingAdapterContractAddress,cycleTotalShares);
            _ibDUSDLendingService.WithdrawBySharesOnly(cycleTotalShares);
                   
            //  Now the Dai is in this contract, transfer it to the treasury contract 
            uint256 balance = _dusd.balanceOf(address(this));
            _dusd.approve(address(_treasuryContract),balance);
            _treasuryContract.depositToken(address(_dusd));
            ...
            _ibDUSD.safeTransfer(address(_esusuAdapterContract),_ibDUSD.balanceOf(address(this)));
    ...
    */
        ...
        // uint256 pricePerFullShare = _ibDUSDLendingService.GetPricePerFullShare();
        
        uint256 overallGrossDaiBalance = currentBalanceShares.mul(_ibDUSDLendingService.GetPricePerFullShare()).div(1e18);
        ...
        //  transfer yDaiShares from the adapter contract to here
        _esusuAdapterContract.TransferIBDUSDSharesToWithdrawalDelegate(currentBalanceShares);

        //  Get the ibDUSDSharesForContractBeforeWithdrawal
        uint ibDUSDSharesForContractBeforeWithdrawal = _ibDUSD.balanceOf(address(this));
        ...
        _ibDUSD.approve(DUSDLendingAdapterContractAddress,currentBalanceShares);
        _ibDUSDLendingService.WithdrawByShares(Mroi,currentBalanceShares);
        ...
        require(ibDUSDSharesForContractBeforeWithdrawal > ibDUSDSharesForContractAfterWithdrawal, "ibDUSD shares before withdrawal must be greater !!!");
        
        //  Update the total balanceShares for this cycle 
        uint256 totalShares = currentBalanceShares.sub(ibDUSDSharesForContractBeforeWithdrawal.sub(ibDUSDSharesForContractAfterWithdrawal));
    ...
    */
    function sendROI(uint256 Mroi, address memberAddress, uint256 esusuCycleId) internal{       
        ...
        //  Send ROI to member 
        _dusd.safeTransfer(memberAddress, memberROINet);
        ...
        _dusd.approve(address(_treasuryContract),finalFee);
        _treasuryContract.depositToken(address(_dusd));

        address cycleOwner = _esusuStorage.GetCycleOwner(esusuCycleId);
        
        _dusd.safeTransfer(cycleOwner, creatorReward);
